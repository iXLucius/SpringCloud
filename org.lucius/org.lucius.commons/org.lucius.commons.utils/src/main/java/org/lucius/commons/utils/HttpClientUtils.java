...
public class HttpClientUtils {
	 ...
	 */
	static {
		requestConfig = RequestConfig.custom().setSocketTimeout(SOCKET_TIME_OUT).setConnectTimeout(CONNECT_TIME_OUT)
				.setConnectionRequestTimeout(CONNECT_TIME_OUT).setCookieSpec(CookieSpecs.STANDARD_STRICT)
		...
		httpClient = HttpClients.custom().setDefaultRequestConfig(requestConfig).setConnectionManager(cm)
				.setRetryHandler(new DefaultHttpRequestRetryHandler(RETRY_TIMES, true)).build();
	...
	public static String post(String url, Map<String, String> header, Map<String, String> params) {
		...
		if (Objects.nonNull(header) && header.size() > 0) {
			header.entrySet().forEach(entry -> headers.add(new BasicHeader(entry.getKey(), entry.getValue())));
		}
	...
	public static String get(String url, Map<String, String> header, Map<String, String> params) {
		...
		return execute(Request.Get(parse(url, params)).setHeaders(headers.toArray(new Header[] {})));
	}
	...
	public static String postMultiple(String url, byte[] fb, Map<String, String> params) throws Exception {
		...
		if (params != null && params.size() > 0) {
			params.entrySet().forEach(entry -> builder.addPart(entry.getKey(),
	...
	public static String postMultiple(String url, MultipartFile mf, Map<String, String> params, String fileName)
			throws Exception {
		MultipartEntityBuilder builder = MultipartEntityBuilder.create();
		builder.setMode(HttpMultipartMode.BROWSER_COMPATIBLE);
		if (params != null && params.size() > 0) {
		...
		builder.addPart(fileName, new ByteArrayBody(mf.getBytes(), mf.getOriginalFilename()));
		HttpEntity entity = builder.build();
		return execute(Request.Post(url).body(entity));
	}
	...
	private static String parse(String url, Map<String, String> params) {
		...
		while (it.hasNext()) {
			String key = it.next();
			try {
				builder.append(key).append(SYMBOL_EQUAL).append(URLEncoder
						.encode(params.get(key) == null ? EMPTY_STRING : params.get(key), Consts.UTF_8.name()))
